...
- Audio/visual: 既存換気素材を4つの固定位置へ配置し、聴取範囲・シーンの表示状態で切替。共通マスターは既存Inspector。機械暴走の旧発光cubeを本編の照明変化へ置換し、階数・正解・期限は維持。8階の不足した806を追加して既定6戸と整合。新しい怪異やルール、物語は加えない。
- Ending: GAME-003にある結果→短い省略可能スタッフロール→タイトルを既存メニューへ接続。既存素材の出典を初期本文とし、作者がページ・文章・表示時間・フェードを編集できる。新しい保存形式・外部通信なし。
- Effects: Homecoming、HallwayLoop／ApartmentRoomTone／CabinPowerPresentation／HomecomingCredits、既存FSM・DemoSessionへの狭い接続、再適用で作者値を上書きしないEditor installer、入力・表示・終了試験。新依存・支出・公開なし。既存作者の文章・音量・比較設定を保護。

## DOOR-001 — 行先未選択時は閉扉待機
- Status/date/owner: 2026-09-20、ユーザー明示指定／Astra実装。
- Decision: 乗車後に8階未選択で閉を押すと、扉を閉じたまま出発せず待機する。開または呼び出しで再開扉、8階選択で通常出発。センサーの挟まり防止を維持する。到着した本物8階の安全再提示と怪異の回答は既定どおり。
- Files/validation: NormalJourneyControllerの乗車待機と到着時再提示を分離。Input Systemによる保持・再開扉・閉扉後出発・閉塞・既存到着を検証する。serialized enum値や作者シーンを書き換えない。

## M4-01 — PC内保存と独立した自己ベスト
- Status/date/owner: 2026-09-20、ユーザーが前回保存案に「良い感じ、続けて実装」と返答。自己ベストはuser-inputで「最短時間と最少死亡数を別々に記録」を選択。
- Decision: M4_SAVE_PLANのPC内版番号付きJSON＋直前バックアップを採用。進行／プロフィールを分離し、最短時間と最少死亡数は独立して更新する。クラウド・新依存なし。ゲーム内文章は既存の作者編集経路へ接続する。
- Scope: GAME-010/013に従う夜境界の続き、死亡・時間の累積、ポーズ／ロード／導入の計時除外、設定保存、結果、既クリア時の導入省略。破損・未来版・書込み不可を黙って上書きしない。Windows再起動の検証まで行う。
...
- Sequence: 導入→柱／設備放送／機械暴走→濡れ／外の声／空間異常→各系統の表現と順序を抽選。早期順は試作で、最終夜は再乗車で再抽選し、死亡でクリア済み夜を巻き戻さない。抽選が偶然同じになることは許容する。
- Timing: 後半2夜の修正猶予は元定義の90%を試作。元assetを書き換えず実行用コピーを作り、次夜・次試行・シーン終了で破棄する。正常な非常停止の追加遭遇は次単位、保存／集計／スタッフロールは後工程。
- Authoring: 全4夜ONなら専用の夜別列を使用。OFFなら作者のRunManager比較列を使用。比較ボタン・空間異常単独メニューは明示的に比較側へ切替。作者の声1件リスト・音量・掲示は保持する。
- Validation: 元定義非変更、固定列・6表現・各系統1回、32seedの抽選構成、旧コピー破棄、入力による全夜帰宅・最終夜死亡再開・後半徒歩救済・既存デモ回帰を確認する。

## M3-03 — かごの天井が遠ざかる空間異常
- Status/date/owner: 2026-09-19、Astra、GAME-005／015・WORK-001内の試作。前工程をユーザーが概ね好評価し自走継続を指定。文章の細かな編集説明は後回し、成果ごとの試すシーン・手順・期待挙動は必ず添える。
- Decision: Hijackの階表示上昇・走行音・非常停止判定をExtend。任意のSpatialCabinPresentationで天井を照明ごと上へ移し、同じ材質の上部壁を伸ばす。初期4m／3秒、悪化時6m／1.4秒。操作盤・床・カメラ・移動領域は固定。
- Alternatives: 奥壁を後退させる案は見える床と実際の移動距離・Colliderの齟齬や救済距離の増加を招く。カメラFOV変化は情報の判別や酔いへ影響する。天井だけの変化なら必要操作を固定できるため採る。汎用変形shaderや外部OSSの導入はこの固定空間では不要。
- Ownership: 専用Prefabに天井Mesh／Materialの共有参照と上部壁を保存。実行時は元天井の描画だけを一時停止し、元のenabled状態を成功・死亡・OnDisableで復元。元Lightを操作盤の可読性のため保持し、上部用の影なしLightを1個追加。正確な動的間接照明の再現ではない。
- Maintainability: 既存シーンを広範囲に組み替えず、元天井の参照パスは日本語Inspectorで明示し、接続切れは変更前に一括検査する。元の天井形状を編集した場合はPrefabの複製側も合わせる必要がある。シーン構造が複数化したら参照方式を再評価する。
- Evidence to seek: 伸長中も階数と音を認識し非常停止できること、放置／誤操作の救済・死亡、ポーズ・復元と通常Hijack互換。技術合格から怖さ・初見理解を認定しない。
...
- Maintainability: 既存Lureの判定・減光・復元をExtendし、見た目だけ差し替える。通常の床shaderへの常時分岐追加は避け、薄い水面専用shaderをBuild。独立した巨大な水シミュレーションや新依存は不要。鏡面だけでは見えづらい場合、最終画面を根拠に濡れの輪郭・広がりを調整する。
- Trial/audio: 外から助けを求める声は仮台詞・仮音声を制作してよい。作者の既存文章を上書きする委任ではない。正式採用・有料サービス・未確認素材の配布は別判断。ローカル音声APIを調査したが、本単位の濡れ実装で音声素材を新規採用してはいない。
- Effects: lure_wet、Lure_WetCorridor、WetFloor材質／shader、比較用RunManager Inspector、関連試験。Homecomingの通常初回は柱を維持。濡れは怪異夜2用候補、全4夜進行と追加遭遇は後続。
- Reflection: 既存PC品質設定はリアルタイム反射off。水面が空を映す問題を実画像で確認し、128pxの室内CubemapをEditorで生成するCustom Probeへ変更。全体の品質設定・常時描画負荷は増やさない。代わりに配置変更時の再ベイクが必要で、減光後の反射は正確な動的鏡像ではない。更新メニューと制約をDEMO_AUTHORINGに記載。
...
- Reason/options: 階数だけでは戸数は決まらず、90戸案よりコンパクトな54戸をユーザーが選択。実物の薄型寸法を採り、9個の大きな箱という不整合を修正。調査根拠と具体値の限界はENTRANCE_REFERENCES。
- Effects: Homecoming、HomecomingSceneBuilder、NearbyInteractionMarkersと作者Inspector、関連入力試験。元デモの作者編集は保持、新依存・物語文章なし。見た目の最終採用は本人評価待ち。

## OPENING-001 — 本編の導入から最初の怪異へ
- Status/date/owner: 2026-09-19、ユーザーが本編冒頭＋代表怪異を先に仕上げる順序を明示承認。user-inputで「帰宅の日常を中心に描く」を選択。
- Context: デモの基盤・小修正から、ゲームの魅力を担う連続した導入と演出へ進む。長い独立映像の先行や、薄い演出のまま全4夜を量産する順序は採らない。
- Decision: M5の導入／演出作業をM3の前段へ移す。仕事帰りの住人が入口から自分で歩き、掲示・設備・生活音に触れ、通常の8階を覚えてから怪異に出会う。人物・会話は増やさない。文章はユーザー執筆、具体配置・タイミングはGAME-015内の試作。
- Effects: 本編用の独立シーンで開始。既存デモと作者編集は保持。導入夜の安全、掲示の有効性、Lureの判別・救済は維持。全4夜・6表現という必須範囲は変更しない。怖さの受入は未了。
- Explicit follow-up: 8階への動機は独白で説明せず、マンション入口の集合郵便受け805を調べる→オートロックが操作可能になる→805を入力→自動扉が開きホールへ、という必須操作で示す。ロッカーは集合郵便受け、▲は近距離で表示する方式をuser-inputで選択。まずこの入口を制作する。805は今回指定された部屋番号であり、既存の自宅8階と整合する。

## DEMO-001 — 利用枠内に導入＋怪異1夜のデモを完成させる
- Status/date/owner: ユーザーが2026-09-15のuser-inputで「導入＋怪異1夜」を明示選択。方針承認であり、実装完了・体験受入ではない。
- Context: 利用枠を使い切る前のプレイアブルデモ完成が必要。選択肢は導入＋怪異1夜、全4夜、M2のみ。
- Decision/reason: 正常帰宅→基本3系統→帰宅・終了を一本化し、開始・ポーズ・設定・終了導線、基本演出・音、必要な見た目、通し検証に集中。全4夜・別3演出・進行保存は本編に残し、デモからは保留。M2のみより正常の学習を保ち、全4夜より制作・回帰範囲を抑える。
- Effects: 作業順はDEMO_PLAN。既存の判定・修正猶予・公平性を保持。全面的な美術作り直し、エンジン移行、新依存、課金・公開は含まない。現在のAstra担当を維持し、モデル変更はしない。ゲーム実装は今回の相談では開始していない。

## WORK-006 — Astra文書に基づく指示・参照の整理

- Status / date / owner: ユーザーの添付文書に沿った改善依頼をCodexが具体化した可逆的な運用変更。2026-09-15、task 01a0a383-4c35-73c0-b769-4876bb5eb297。個々の編集案をユーザーが個別選択したという記録ではない。
- Context / options: 毎回の仕様全文読込、重複する手順、PROJECT_STATUS内の古い現在地、Unityスキルの旧hold指示を確認。常時全文読込を維持する案より、目的別参照と現在／履歴の分離を採用する。
- Decision / reason: AGENTSとSTART_HEREを入口にし、必要な規則・例外・決定を読む。PROJECT_STATUSを現在地に絞り、旧本文はPROJECT_HISTORYへ保存。スキルは適用条件と固有の注意・参照に絞る。変更の影響に応じた検証、回帰修正、記録・保存までを完了条件にする。
- Continuity: WORK-001/002/005・GAME-015の委任は保持。WORK-003の通常Sol運用案より、VIS-004以降のAstra継続という最新の明示指定を優先する。限定レビュー以外で機械的にSolへ戻す停止文を使わない。
- Effects / limits: AGENTS、START_HERE、PROJECT_STATUS／HISTORY、VALIDATION、DEVELOPMENT、OPEN_QUESTIONS、プロジェクト内2スキル。ゲーム仕様・コード・モデル設定・サブエージェント方針・全プロジェクト用の個人設定は変更しない。重大判断・製品条件・人の体験受入を省略する委任ではない。
- Evidence: 根拠文書、監査所見、検証と限界は [WORKFLOW_AUDIT_2026-09-15.md](WORKFLOW_AUDIT_2026-09-15.md)。時間・利用枠削減の実測値はない。
...
## WORK-003 — モデル運用の簡素化
- 現行の補足: VIS-004以降のAstra継続指定を優先。WORK-006で参照方法と完了条件を整理。以下の通常運用案を理由に、明示された継続担当を自動で変更しない。
...
## FEEDBACK-001 — 実プレイ指摘9点への修正（2026-09-15）

- ユーザー指定: 革靴／コンクリート足音、静かな走行・扉音とInspector調整、内外から読める扉目地、扉中央の水平線除去、チャイム後の間、8ボタンの登録灯、出入口を向いた右袖壁上部のモニター、早期ダッシュ退出の誘引Reveal、13階上限を廃した一定ペースの上昇。
- 参考: 三菱電機 [ボタン外周リング](https://www.mitsubishielectric.co.jp/corporate/chiteki/otb/list/089/index.html)、東芝 [かご内サイネージの設置例・寸法](https://www.toshiba-elevator.co.jp/elv/maintenance/d_signage/)、東芝 [低振動・低騒音への更新](https://www.toshiba-elevator.co.jp/elv/renewal/purpose/)。実機の同一音の再現や測定値とは扱わない。
- 音: 実録素材ShoeLeatherSteps.wav（HerbertBoland、CC BY 4.0）から4歩を切り出し、低域除去・端部fade・音量調整。出典と加工内容は `docs/licenses/Footsteps.txt`。扉音は既存生成音の強い高調波／7Hz変調／広帯域摩擦を除いて静かな駆動層へ変更。走行音は同じ波形の音量を0.55→0.16へ。音量の初期値はユーザー試聴で調整する試作値。
- 設備: モニターの筐体背面をFrontLeftの室内面へ2mm重ねて固定。出入口を向くと右、主操作盤の反対側。表示文は従来の文章編集を維持。扉は内側にも各可動葉に追従する目地を追加。扉材質のみ写真テクスチャを鏡像反復し、繰り返し境界の水平線を解消する。
- 登録灯: 有効な8階指定で3操作盤の8外周が同期点灯。到着・夜リセットで消灯、閉扉中断時は登録を解除。無効な階ボタンを新たな行先として実装しない。
- 到着: チャイムから開扉まで1.4秒を初期値としInspector編集可。通過中の通常ボタン受付は増やさない。
- 怪異: 開扉中の退出を拾ってDiagnosisへ進みRevealする。13階固定のdriftとReveal時の加速を廃し、1秒ごとの階数上昇を継続。既存の初回期限・修正猶予・死亡は維持し、非常停止／遭遇終了で停止する。
- 付随修正: ユーザーが文章編集rootを改名した場合もコンポーネントで識別し、内装Apply時の重複コレクション生成を防ぐ。ユーザー執筆内容と名称を保持する。
